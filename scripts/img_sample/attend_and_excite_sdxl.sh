#!/bin/bash
#SBATCH --job-name=sdxl_sample-ae_test_add_freespace ## job name
...
#SBATCH --output=logs/test/ae_test_add_freespace_3.log
...
accelerate launch --multi_gpu --num_processes 4 sdxl_sample.py \
    --total_images 8 \
    --out_dir samples/test/ae/cls${class_count}/seed${seed}_add_freespace_3 \
    --seed ${seed} \
