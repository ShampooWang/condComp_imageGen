#!/bin/bash
#SBATCH --job-name=sdxl_sample-condComp_attnScore_3 ## job name
...
#SBATCH --output=logs/test/condComp_attnScore_3.log

class_count=2
seed=322
accelerate launch --multi_gpu --num_processes 4 sdxl_sample.py \
    --total_images 8 \
    --out_dir samples/test/seq_inpaint_val/cls${class_count}/seed${seed}_4 \
    --seed ${seed} \
